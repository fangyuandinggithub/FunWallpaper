...
- (void)buildUI{
    ...
    _toolBar = [[XLImageViewerTooBar alloc] initWithFrame:CGRectMake(0, self.bounds.size.height - ToolBarHeight - 20, self.bounds.size.width, ToolBarHeight)];
    __weak typeof(self)weekSelf = self;
    [_toolBar addSaveBlock:^{
        [weekSelf saveImage];
    }];
    [_toolBar setPreViewBlock:^{
        if (weekSelf.previewImageView.image == nil) {
            weekSelf.previewImageView.image = weekSelf.homeImage;

        } else if (weekSelf.previewImageView.image == weekSelf.homeImage) {
            weekSelf.previewImageView.image = weekSelf.lockScreenImage;
        } else {
            weekSelf.previewImageView.image = nil;
        }
        
    }];
    [_toolBar setEditViewBlock:^{
