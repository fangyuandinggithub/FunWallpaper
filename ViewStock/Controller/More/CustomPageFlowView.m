...
#import "CustomPageFlowView.h"
@interface CustomPageFlowView ()
...
@implementation CustomPageFlowView
...
- (void)initialize{
    ...
    subviewClassName = @"CustomPageFlowView";
...
- (void)queueReusableCell:(CustomIndexBannerSubview *)cell{
    [_reusableCells addObject:cell];
}

- (void)removeCellAtIndex:(NSInteger)index{
    CustomIndexBannerSubview *cell = [_cells objectAtIndex:index];
    ...
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                CustomIndexBannerSubview *cell = [_cells objectAtIndex:i];
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    CustomIndexBannerSubview *cell = [_cells objectAtIndex:pageIndex];
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        cell.didSelectCellBlock = ^(NSInteger tag, CustomIndexBannerSubview *cell) {
...
- (void)reloadData {
    ...
    for (UIView *view in self.scrollView.subviews) {
        if ([NSStringFromClass(view.class) isEqualToString:subviewClassName] || [view isKindOfClass:[CustomIndexBannerSubview class]]) {
...
- (CustomIndexBannerSubview *)dequeueReusableCell{
    CustomIndexBannerSubview *cell = [_reusableCells lastObject];
    if (cell)
    {
...
//点击了cell
- (void)singleCellTapAction:(NSInteger)selectTag withCell:(CustomIndexBannerSubview *)cell {
